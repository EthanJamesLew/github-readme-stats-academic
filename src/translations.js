...
/**
 * Retrieves academic stat card labels in the available locales.
 *
 * @param {string} name The name of the locale.
 * @param {string} apostrophe Whether to use apostrophe or not.
 * @returns {Object} The locales object.
 */
const academicStatCardLocales = ({ name, apostrophe }) => {
  const encodedName = encodeHTML(name);
  return {
    "academiccard.title": {
      en: `${encodedName}'${apostrophe} GitHub Impact`,
    },
    "academiccard.totalstars": {
      ar: "مجموع النجوم",
      cn: "获标星数（star）",
      "zh-tw": "獲標星數（star）",
      cs: "Celkem hvězd",
